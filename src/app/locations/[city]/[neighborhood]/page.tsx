...
export default async function Page({params}:{params:Promise<{city:string;neighborhood:string}>}){
    ...
    '@graph': [
      {
        '@type': 'LocalBusiness',
        name: 'RenewShine',
        url: `https://renewshine.co/locations/${n.citySlug}/${n.slug}`,
    ...
    <>
      ...
      <section className="bg-white border-b border-slate-200 py-16">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-sm text-slate-700 font-medium">{n.cityName}</span>
            <span className="text-slate-300">/</span>
            <span className="text-sm text-slate-700 font-medium">{n.name}</span>
          </div>
          ...
          <div className="max-w-3xl">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-(--color-brand-muted) px-3 py-1 text-xs font-semibold uppercase tracking-wide text-(--color-brand) mb-4">
              Premium Cleaning · {n.cityName}
            </span>
            <h1 className="font-display text-4xl font-bold text-slate-900 md:text-5xl">{n.h1}</h1>
            <p className="mt-4 text-lg text-slate-600 leading-relaxed max-w-2xl">{n.intro}</p>
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl mb-8">
            <p className="text-xs font-semibold uppercase tracking-wider text-(--color-brand)">
              Homes we clean in {n.name}
            </p>
            <h2 className="mt-2 font-display text-2xl font-bold text-slate-900">Property types we serve</h2>
            <p className="mt-2 text-slate-600">{n.housingNote}</p>
          </div>
      ...
      <section className="bg-white py-16">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="mx-auto mb-10 max-w-2xl text-center">
            ...
            <h2 className="mt-2 font-display text-3xl font-bold text-slate-900">Cleaning services in {n.name}</h2>
      ...
      <section className="bg-slate-50 py-14">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="mt-4 text-sm font-semibold text-slate-900">— {n.testimonial.name}</p>
          <p className="text-sm text-slate-600">{n.testimonial.location}</p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
          <div className="mb-10 text-center">
            ...
            <h2 className="mt-2 font-display text-2xl font-bold text-slate-900">
              Questions about cleaning in {n.name}
            </h2>
          </div>
      ...
      <section className="bg-(--color-brand) py-16">
        <div className="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
          <h2 className="font-display text-3xl font-bold text-white">Ready for a clean in {n.name}?</h2>
