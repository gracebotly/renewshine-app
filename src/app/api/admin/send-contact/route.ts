...
import { sendContactPhotos } from '@/lib/email'
...
export async function POST(request: Request) { /* trimmed for brevity in command */
  ...
  if (!['email', 'sms'].includes(method)) return Response.json({ error: 'method must be email or sms' }, { status: 400 })
