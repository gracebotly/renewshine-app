...
export async function POST(request: Request) {
  ...
  const { jobId, approvedPrice, depositAmount, confirmedDate, regenerate, channel = 'email', customSmsBody, recurringFrequency } = await request.json()
...
` +
        ...
        if (regenerate) {
          ...
          await sendCustomerQuote(updatedJob, paymentLink.url, resolvedDeposit, recurringFrequency as string | undefined)
        }
      }
    }
  } catch (sendError) {
