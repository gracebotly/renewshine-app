import type { Job } from '@/types/database'
import { baseTemplate, badge, heading, para, ctaButton } from './base'

export function customerQuoteTemplate(
  job: Job,
  stripeUrl: string
): { subject: string; html: string } {
  const firstName = job.client_name.split(' ')[0]
  const subject = `Your RenewShine quote is ready, ${firstName}`

  const timePrefMap: Record<string, string> = {
    morning:         '8am – 12pm',
    afternoon:       '12pm – 5pm',
    early_morning:   '8am – 10am',
    mid_morning:     '10am – 12pm',
    noon:            '12pm – 2pm',
    early_afternoon: '2pm – 4pm',
    late_afternoon:  '4pm – 6pm',
    flexible:        'Flexible',
  }
  const timePref = job.availability_time_pref
    ? (timePrefMap[job.availability_time_pref] ?? 'Flexible')
    : 'Flexible'

  const confirmedDateStr = job.confirmed_date
    ? new Date(job.confirmed_date).toLocaleDateString('en-US', {
        weekday: 'long', month: 'long', day: 'numeric', year: 'numeric',
      })
    : ''

  const approvedPrice = job.approved_price ?? 0
  const depositAmount = 100
  const remainingAmount = Math.max(approvedPrice - depositAmount, 0)

  const priceDisplay = approvedPrice > 0
    ? `$${approvedPrice.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
    : 'Confirmed after review'

  const lineItems: Array<{ description: string; amount: string }> =
    Array.isArray((job as any).quote_line_items) && (job as any).quote_line_items.length > 0
      ? (job as any).quote_line_items
      : []

  const serviceLabel =
    job.service_type === 'standard'            ? 'Standard Clean'
    : job.service_type === 'deep'              ? 'Deep Clean'
    : job.service_type === 'move_out'          ? 'Move-In / Move-Out'
    : job.service_type === 'post_construction' ? 'Post-Construction'
    : 'Cleaning Service'

  const bedroomLine =
    job.service_type !== 'move_out' && job.bedrooms
      ? `${job.bedrooms} bed / ${job.bathrooms} bath`
      : ''

  const lineItemRows = lineItems.length > 0
    ? lineItems.map((item) => `
      <tr>
        <td style="padding:10px 0;font-size:13px;color:#334155;border-bottom:1px solid #f1f5f9;">
          ${item.description}
        </td>
        <td style="padding:10px 0;font-size:13px;color:#0f172a;font-weight:600;font-family:'Courier New',monospace;text-align:right;border-bottom:1px solid #f1f5f9;">
          $${parseFloat(item.amount).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
        </td>
      </tr>`
    ).join('')
    : `<tr>
        <td style="padding:10px 0;font-size:13px;color:#334155;">
          <span style="display:inline-block;width:6px;height:6px;border-radius:50%;background:#4A7C59;margin-right:8px;vertical-align:middle;"></span>
          ${serviceLabel}${bedroomLine ? ` · ${bedroomLine}` : ''}
        </td>
        <td style="padding:10px 0;font-size:13px;color:#0f172a;font-weight:600;font-family:'Courier New',monospace;text-align:right;">
          ${priceDisplay}
        </td>
      </tr>`

  const content = `
    ${badge('Your quote is ready', 'green')}
    ${heading(`Your quote is ready, ${firstName}.`)}
    ${para('We reviewed your submission and confirmed your price. Your date and deposit link are below.')}

    <table width="100%" cellpadding="0" cellspacing="0" role="presentation"
      style="border-left:4px solid #4A7C59;background:#f4f7f5;border-radius:0 8px 8px 0;margin:0 0 24px;overflow:hidden;">
      <tr>
        <td style="padding:16px 20px;">
          <p style="margin:0 0 4px;font-size:16px;font-weight:700;color:#0f172a;">${confirmedDateStr}</p>
          <p style="margin:0;font-size:13px;color:#64748b;">${timePref}&nbsp;&nbsp;·&nbsp;&nbsp;${job.address ?? ''}</p>
        </td>
      </tr>
    </table>

    <p style="margin:0 0 8px;font-size:12px;font-weight:700;color:#0f172a;text-transform:uppercase;letter-spacing:0.06em;">What's included</p>
    <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="margin:0 0 0;">
      <tbody>
        ${lineItemRows}
      </tbody>
    </table>

    <table width="100%" cellpadding="0" cellspacing="0" role="presentation"
      style="margin:0 0 24px;border-top:2px solid #0f172a;">
      <tbody>
        <tr>
          <td style="padding:12px 0 4px;font-size:15px;font-weight:700;color:#0f172a;">
            Total
          </td>
          <td style="padding:12px 0 4px;font-size:15px;font-weight:700;color:#0f172a;font-family:'Courier New',monospace;text-align:right;">
            ${priceDisplay}
          </td>
        </tr>
        <tr>
          <td style="padding:2px 0;font-size:12px;color:#64748b;">
            Deposit due today
          </td>
          <td style="padding:2px 0;font-size:12px;color:#64748b;font-family:'Courier New',monospace;text-align:right;">
            $${depositAmount.toFixed(2)}
          </td>
        </tr>
        <tr>
          <td style="padding:2px 0 0;font-size:12px;color:#64748b;">
            Remaining balance (due on day of service)
          </td>
          <td style="padding:2px 0 0;font-size:12px;color:#64748b;font-family:'Courier New',monospace;text-align:right;">
            $${remainingAmount.toFixed(2)}
          </td>
        </tr>
      </tbody>
    </table>

    <table width="100%" cellpadding="0" cellspacing="0" role="presentation"
      style="border-left:4px solid #4A7C59;background:#f4f7f5;border-radius:0 8px 8px 0;margin:0 0 24px;overflow:hidden;">
      <tr>
        <td style="padding:14px 18px;">
          <p style="margin:0;font-size:13px;color:#334155;line-height:1.65;">
            If anything looks different when we arrive, we'll contact you before proceeding. <strong style="color:#0f172a;">No surprise charges.</strong>
          </p>
        </td>
      </tr>
    </table>

    <p style="margin:0 0 6px;font-size:12px;color:#94a3b8;text-align:center;">Your date is not held until your deposit is paid. This quote is valid for 48 hours.</p>

    ${ctaButton('Pay $100 deposit — confirm my appointment', stripeUrl)}
  `

  return {
    subject,
    html: baseTemplate(
      content,
      `Your confirmed price is ${priceDisplay}. Pay your $100 deposit to lock in your date.`
    ),
  }
}
