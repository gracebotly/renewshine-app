 ...
 * Fires: when owner regenerates a Stripe link via "Resend Link" and the
 *        original link was expired (job.status === 'approved', deposit_paid === false)
...
export function customerLinkExpiredTemplate(
  ...
  const subject = `${firstName}, your booking link expired — here's a new one`
  ...
  const content = `
    ${badge('New Payment Link', 'navy')}
    ${heading(`${firstName}, your previous link expired.`)}
    ${para(`No problem — we've generated a fresh payment link for your booking. Your confirmed price of $${job.approved_price} and your date are still held for you.`)}
    ...
    <div style="margin:20px 0;padding:14px 16px;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;">
      ...
      <p style="margin:6px 0 0;font-size:13px;color:#166534;">
        This new link is valid for 48 hours. Pay the $${deposit} deposit to lock it in.
      </p>
    </div>

    ${trustStrip()}
    ${divider}
    ${ctaButton('Pay $100 Deposit — Lock In My Date →', newStripeUrl)}

    <p style="margin:16px 0 0;font-size:12px;color:#94a3b8;text-align:center;">
      Questions? Reply to this email and we'll sort it out quickly.
    </p>
  `
  ...
  return {
    ...
    html: baseTemplate(
      content,
      `Your RenewShine payment link expired — here's a fresh one. Your price and date are unchanged.`
    ),
  }
}
