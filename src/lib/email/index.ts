...
export async function sendCustomerQuote(job: Job, stripeUrl: string, depositAmount?: number, recurringFrequency?: string): Promise<void> {
  const { subject, html } = customerQuoteTemplate(job, stripeUrl, depositAmount, recurringFrequency)
  await resend.emails.send({ from: FROM, to: job.client_email, subject, html })
}
