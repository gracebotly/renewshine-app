import { baseTemplate, badge, heading, ctaButton, divider } from '@/lib/email/templates/base'
...
function t(value: string, ctx: RenderContext): string { return escapeHtml(renderTemplate(value, ctx.tokens)) }
...
function paragraphHtml(text: string, ctx: RenderContext): string {
  return renderTemplate(text, ctx.tokens).trim().split(/\n{2,}/).map(p =>
...
function renderBlock(block: EmailBlock, ctx: RenderContext): string {
  switch (block.type) {
    case 'badge': return badge(t(block.text, ctx), block.color)
    case 'heading': return heading(t(block.text, ctx))
    case 'paragraph': return paragraphHtml(block.text, ctx)
    case 'divider': return divider
    case 'scheduleCard': {
      const pill = block.pill?.trim()
      return `
    ...
    case 'detailsCard': {
      const addOnRows = ctx.addOnLabels.map(label => `
      <tr>
    ...
    case 'invoiceTable': {
      if (ctx.invoiceLines.length === 0 && ctx.quoteLineItems.length === 0) return ''
      const rows = ctx.invoiceLines.length > 0
    ...
    case 'bulletList': {
      const rows = block.lines.map(line => renderTemplate(line, ctx.tokens).trim()).filter(Boolean)
        .map(line => `<tr><td style="padding:3px 0;font-size:13px;color:#0f172a;line-height:1.5;"><span style="color:#4A7C59;font-weight:700;margin-right:8px;">·</span>${escapeHtml(line)}</td></tr>`).join('')
      if (!rows) return ''
      return `
    <table width="100%" cellpadding="0" cellspacing="0" role="presentation"
    ...
    case 'numberedSteps': {
      const lines = block.lines.map(l => renderTemplate(l, ctx.tokens).trim()).filter(Boolean)
    ...
    case 'button': return ctaButton(t(block.label, ctx), resolveUrl(block.target, block.url, ctx))
  }
}

export function renderEmailDocument(doc: EmailDocument, ctx: RenderContext): { subject: string; html: string } {
  const content = doc.blocks.map(b => renderBlock(b, ctx)).join('\n')
  const firstHeading = doc.blocks.find(b => b.type === 'heading')
  const preview = firstHeading ? renderTemplate(firstHeading.text, ctx.tokens) : renderTemplate(doc.subject, ctx.tokens)
  return { subject: renderTemplate(doc.subject, ctx.tokens), html: baseTemplate(content, escapeHtml(preview)) }
}
